...
export const bitrateService = async (fileName: string) => {
  ...
  resolutions.forEach(resolution => {
    ['plain', 'watermarked'].forEach(type => {
      const watermarkedPath = path.join(__dirname, '../..', 'assets', 'watermark.png');
      console.log(`Starting conversion of ${type} to ${resolution.name}`);
      const outputDir = path.join(convertedDir, resolution.name);
      ...
      const options = [
        ...
        `-hls_segment_filename`, `${outputDir}/${type}%03d.ts`,
      ]

      const command = ffmpeg().input(filePath)
        .output(
          path.join(outputDir, `${type}.m3u8`))
      if (type === 'watermarked') {
        command.input(watermarkedPath)
          .complexFilter([
            {
              filter: 'scale',
      ...
      command.outputOptions(options)
        .on('progress', (progress) => {
          console.log(`Processing: ${type} ${resolution.name} = ${progress.percent} % done`);
        })
        .on('end', () => {
          console.log(`Converted to ${type} HLS ${resolution.name}`);
        })
        .on('error', (err) => {
          console.error(`Error converting ${type} to HLS ${resolution.name}: `, err.message);
        })
        .run();
